...
type
  ...
  TCompRes = object
    kind: TResKind
    typ: TJSTypeKind
    res: ESNode               # result part; index if this is an
                             # (address, index)-tuple
    address: ESNode           # address of an (address, index)-tuple
    tmpLoc: ESNode            # tmp var which stores the (address, index)
  ...
  PGlobals = ref object of RootObj
    typeInfo, constants, code: ESNode
    forwarded: seq[PSym]
...
proc rdLoc(a: TCompRes): ESNode {.inline.} =
  if a.typ != etyBaseIndex:
    result = a.res
  else:
    result = newMemberExpr(a.address, a.res, false)
...
proc nimObjDesc(kind:int, len: Natural, offset, typ, name, sons: ESNode) : ESNode =
  newObjectExpr([
    newProperty(newESIdent("kind"), newESLiteral(kind)),
    newProperty(newESIdent("len"), newESLiteral(len)),
    newProperty(newESIdent("offset"), offset),
    newProperty(newESIdent("typ"), if typ.isNil: newESEmitExpr("null") else: typ),
    newProperty(newESIdent("name"), if name.isNil: newESEmitExpr("null") else: name),
    newProperty(newESIdent("sons"), if sons.isNil: newArrayExpr() else: sons)
  ])

proc genTypeInfo(p: PProc, typ: PType): ESNode
proc genObjectFields(p: PProc, typ: PType, n: PNode): ESNode =
  var
    s, u: ESNode
    field: PSym
    b: PNode
  ...
  of nkRecList:
    if n.len == 1:
      ...
      result = nimObjDesc(2, n.len, newESLiteral(0), nil, nil, s)
  of nkSym:
    field = n.sym
    ...
    result = nimObjDesc(
      1, 
      0, 
      newESLiteral($mangleName(p.module, field)), 
      s,
      newESLiteral($makeJSString(field.name.s)),
      nil
    )
  of nkRecCase:
      ...
      of nkOfBranch:
        ...
        for j in 0..<b.len - 1:
          if b[j].kind == nkRange:
            ...
            u.add( newArrayExpr([
              newESLiteral(getOrdValue(b[j][0].toInt),
              newESLiteral(getOrdValue(b[j][1]).toInt)])
          else:
            u.add(newESLiteral(getOrdValue(b[j])))
      of nkElse:
